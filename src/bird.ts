...
export class Bird {
	...
	async load(app: Application, base: Sprite) {
		...
		this.inGameLowerBound =
			app.screen.height - base.height - this.sprite.height / 2 - 3;
	}

	updateGameOver(_app: Application, _ctx: State) {
		return;
	}

	updatePreGame(_app: Application, _ctx: State) {
	...
	updateInGame(_app: Application, ctx: State) {
	...
	constructor(app: Application) {
		...
		this.idleLowerBound = (app.screen.height / 5) * 2.9;
		this.textures = [];
		this.tempUpperBound = 0;
